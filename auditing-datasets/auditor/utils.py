...
def str_to_time(timestamp,tzsource=None):
    ...
    try:
        # parse timestamp
        dtobj = parse(timestamp)
    except:
        return None

    
    # extract timezone from the datetime object you parsed, check if null
    if dtobj.tzinfo is None:
        # use tzsource timezone
        if tzsource is None:
            return dtobj
        elif type(tzsource) == datetime.datetime:
            #extract timezone
            newobject = dtobj.replace(tzinfo=tzsource.tzinfo)
            return newobject
        else:
            # not a datetime obj, use pytz for time zone
            myzone = pytz.timezone(tzsource)
            newobject = myzone.localize(dtobj)
            return newobject
    else:
        # ignore tzsource timezone since timestamp has timezone value
...
def daytime(time,daycycle):
    ...
    # Return None if any necessary info is missing
    if year not in daycycle:
        return None
    if monthday not in daycycle[year]:
        return None
    if 'sunset' not in daycycle[year][monthday]:
        return None

    # extract sunrise and sunset values
    sunset_str = date +' '+ daycycle[year][monthday]['sunset']
    sunrise_str = date +' '+ daycycle[year][monthday]['sunrise']


    sunset_obj = str_to_time(sunset_str,daycycle['timezone'])
    sunrise_obj = str_to_time(sunrise_str,daycycle['timezone'])
    ...
    if time.tzinfo is not None:
        ...
        time = str_to_time(time_str,tz)
    
    result = ((time > sunrise_obj) and (time < sunset_obj))
